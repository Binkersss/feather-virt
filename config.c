...
int config_set_image(container_config_t *cfg, const char *image_name) {
    ...
    snprintf(tarball_path, MAX_PATH_LEN, "%s/%s.tar.gz", DEFAULT_IMAGES_DIR, image_name);
    
    /* Validate tarball exists */
    if (stat(tarball_path, &st) != 0) {
        fprintf(stderr, "Error: Image '%s' not found at %s\n", image_name, tarball_path);
        fprintf(stderr, "Hint: Run 'scripts/build_rootfs.sh build <distro> <version>' to build images\n");
        return -1;
    }
...
void config_list_images(const char *images_dir) {
    ...
    if (!dir) {
        ...
        fprintf(stderr, "Hint: Run 'scripts/build_rootfs.sh build-all' to create images\n");
        return;
    }
