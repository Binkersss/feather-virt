...
int config_save_to_file(const container_config_t *cfg) 
{
    ...
    json_object_object_add(root, "name", json_object_new_string(cfg->name));
    ...
    json_object_object_add(root, "image", json_object_new_string(image_name));
    
    /* Store full path for internal use */
    json_object_object_add(root, "base_root", json_object_new_string(cfg->base_root));
    
    /* Format timestamp as ISO 8601 */
    char time_buf[64];
    struct tm *tm_info = gmtime(&cfg->created_at);
    strftime(time_buf, sizeof(time_buf), "%Y-%m-%dT%H:%M:%SZ", tm_info);
    json_object_object_add(root, "created_at", json_object_new_string(time_buf));
    ...
    json_object_object_add(limits, "memory_bytes", 
                          json_object_new_int64(cfg->limits.memory_bytes));
    ...
    /* Add status */
    json_object_object_add(root, "status", json_object_new_string(cfg->status));
    
    /* Add exit code if container has exited */
    if (strcmp(cfg->status, "exited") == 0 || strcmp(cfg->status, "killed") == 0) {
        json_object_object_add(root, "exit_code", json_object_new_int(cfg->exit_code));
    }
...
int config_load_from_file(const char *config_path, container_config_t *cfg) 
{
    ...
    if (json_object_object_get_ex(root, "base_root", &obj)) {
        strncpy(cfg->base_root, json_object_get_string(obj), MAX_PATH_LEN - 1);
    ...
    if (json_object_object_get_ex(root, "command", &obj)) {
        if (json_object_is_type(obj, json_type_array) && 
            ...
            strncpy(cfg->shell, json_object_get_string(cmd), MAX_PATH_LEN - 1);
        }
    }
    ...
    if (json_object_object_get_ex(root, "status", &obj)) {
        strncpy(cfg->status, json_object_get_string(obj), sizeof(cfg->status) - 1);
    }
...
int config_list_containers(int show_all) 
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        snprintf(config_path, sizeof(config_path),
                 "%s/%s/config.json", DEFAULT_CONTAINER_BASE, entry->d_name);
