...
void build_cgroup_path(const char *base, const char *name, char *output,
		       size_t output_len)
{
...
void build_memory_limit_string(unsigned long long mb, char *output,
			       size_t output_len)
{
...
void setup_cgroup(const container_config_t *cfg)
{
    ...
    // Build cgroup path
    build_cgroup_path(cfg->cgroup_base, cfg->name, cgroup_path,
		      sizeof(cgroup_path));
