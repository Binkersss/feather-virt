...
int child_main(void *arg) {
    ...
    if (r <= 0) {
        fprintf(stderr, "[child] failed to read merged path from parent\n");
        return 1;
    }
    close(sync_pipe[0]);
    sync_pipe[0] = -1;
...
void print_usage(const char *prog) {
    ...
    printf("\nOptions:\n");
    printf("  --image <name>        Select base rootfs image (required unless --list-images)\n");
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "i:ls:n:h", long_options, NULL)) != -1) {
        switch (opt) {
            case 'i':
                image_name = optarg;
                break;
