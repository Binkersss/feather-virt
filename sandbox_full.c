...
static void setup_minimal_dev() {
    ...
    if (mknod("/dev/null", S_IFCHR | 0666, makedev(1,3)) && errno != EEXIST) perror("mknod /dev/null");
    if (mknod("/dev/zero", S_IFCHR | 0666, makedev(1,5)) && errno != EEXIST) perror("mknod /dev/zero");

    mkdir_p("/dev/pts", 0755);
    if (mount("devpts", "/dev/pts", "devpts", 0, NULL) != 0) perror("mount devpts");
    if (mknod("/dev/ptmx", S_IFCHR | 0666, makedev(5,2)) && errno != EEXIST) perror("mknod /dev/ptmx");
}
...
static void setup_cgroup(pid_t child) {
    ...
    FILE *f = fopen(path, "w");
    if (f) { fprintf(f, "%llu", (unsigned long long)128*1024*1024); fclose(f); }
...
static int setup_overlay_root_for_pid(pid_t pid, char *merged_out, size_t merged_len) {
    char upper[512], work[512], merged[512], container_dir[512];

    snprintf(container_dir, sizeof(container_dir), "%s/%d", container_base, (int)pid);
    ...
    snprintf(opts, sizeof(opts), "lowerdir=%s,upperdir=%s,workdir=%s", base_root, upper, work);
...
int child_main(void *arg) {
    ...
    if (r <= 0) {
        fprintf(stderr, "[child] failed to read merged path from parent: %s\n", strerror(errno));
        return 1;
    }
    ...
    if (chroot(merged_path) != 0) { perror("chroot failed"); return 1; }
    ...
    if (sethostname("sandbox", strlen("sandbox")) != 0) perror("sethostname");
...
int main(int argc, char *argv[]) {
    ...
    pid_t child = clone(child_main, child_stack + STACK_SIZE, flags, (void*)shell);
    if (child < 0) { perror("clone failed"); exit(1); }
    ...
    f = fopen(map_file, "w");
    if (f) { fwrite("deny\n", 1, 5, f); fclose(f); } /* deny setgroups if supported */
    ...
    if (setup_overlay_root_for_pid(child, merged_root, sizeof(merged_root)) != 0) {
    ...
    if (umount(merged_root) != 0) perror("umount merged_root");
    /* remove container directory tree (upper, work, rootfs) */
    char rmcmd[1024];
    snprintf(rmcmd, sizeof(rmcmd), "/bin/sh -c 'rm -rf %s/%d'", container_base, (int)child);
    system(rmcmd);
