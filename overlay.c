...
int setup_overlay_root(const container_config_t *cfg, char *merged_out, size_t merged_len) {
...
void cleanup_overlay(const container_config_t *cfg, const char *merged_root) {
    char rmcmd[1024];
    char container_dir[MAX_PATH_LEN];
    ...
    snprintf(rmcmd, sizeof(rmcmd), "/bin/sh -c 'rm -rf %s'", container_dir);
    system(rmcmd);
}
